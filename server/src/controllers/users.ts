...
export default class UsersController {
	public static findById = async (id: string, build = false): Promise<UserModel | UserDTO> => {
		return new Promise<UserModel | UserDTO>((resolve, reject) => {
			User.findById(id, (err, user: UserModel) => {
				if (err) {
					reject(Errors.databaseAccess(err));
				} else if (!user) {
					reject(Errors.ressourceNotFound({type: 'user', id}));
				} else if (build) {
	...
	public static save = async (user: Document, build = false): Promise<UserModel | UserDTO> => {
		return new Promise<UserModel | UserDTO>((resolve, reject) => {
